...
export default function NuroDashboard({ onClose }) {
    ...
    const data = {
        score: Math.round(lastCollab.overallScore || memory?.trustSnapshot?.compositeScore || 70),
        delta: lastCollab.scoreDelta || 0,
        positives: lastCollab.positives || ["Consistent communication"],
        negatives: lastCollab.negatives || [],
        rootCause: lastCollab.rootCause || "Limited data. Start more collaborations to unlock deep insights.",
        fixes: lastCollab.fixes || [],
        prediction: lastCollab.predictedSuccessProbability || 70,
        comparison: {
            clarity: { past: 50, current: memory?.metrics?.communicationClarity || 50 },
            reliability: { past: 50, current: memory?.metrics?.reliabilityScore || 50 },
            trust: { past: 50, current: memory?.metrics?.trustIndex || 50 }
        }
    };
